...
from nupic.bindings.regions.CounterSource import CounterSource
from nupic.bindings.regions.SimpleSink import SimpleSink
...
class PyRegionTest(unittest.TestCase):
  ...
  def testDelayedLinks(self):
    Network.registerPyRegion(CounterSource.__module__, CounterSource.__name__)
    Network.registerPyRegion(SimpleSink.__module__, SimpleSink.__name__)

    net = Network()
    net.addRegion("source", "py.CounterSource", "")
    net.addRegion("sink", "py.SimpleSink", "")
